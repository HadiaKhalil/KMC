...
export default function ClientSection() {
  ...
  const handleNext = () => setCurrentIndex((prev) => (prev + 1) % clientsData.length);
  const handlePrev = () => setCurrentIndex((prev) => (prev - 1 + clientsData.length) % clientsData.length);
        ...
        <div className="fixed inset-0 bg-black/90 z-[9999] flex flex-col">
          <div className="flex justify-end p-4">
            <button
              ...
              className="text-white text-3xl hover:text-[#EE7A30] transition"
            >
              <MdClose />
            </button>
          </div>
